...
flatpickr(datePickerInput, {
  dateFormat: "Y-m-d",
  defaultDate: "2017-01-01", // ⬅️ This line
  disable: [
  function(d) {
    const s = formatLocalDate(d);
    return !testDates.includes(s);
  }
],


  
  monthSelectorType: "static", // ⬅️ This line
  onChange: (sd, ds) => {
    selectedDate = ds;
    confirmDateBtn.disabled = !ds;
  },
 onDayCreate: (dObj, dStr, fp, dayElem) => {
  ...
  readyBtn.addEventListener("click", () => {
    chatbotDialog.classList.remove("visible");
    chatbotReady = true;
  });

  chatbotIcon.addEventListener("click", () => {
    chatbotDialog.classList.toggle("visible");
  });

  // === Start Test (open calendar) ===
  startBtn.addEventListener("click", () => {
    if (!chatbotReady) {
      alert("Please click 'Ready!' in the chatbot first.");
      return;
    }
    ...
    .then(data => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          iframe.src = "https://real-time-load-curve-dashboard-backend.onrender.com/dashboard"; // ✅ Corrected
